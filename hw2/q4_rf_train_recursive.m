function [tree, next_slot] = q4_rf_train_recursive(X, Y, feat_idx, curr_tree, slot, c, F)
% Learns recursively a *random* decision tree using the examples (X,Y) falling in the current node.
% This function is used by q4_rf_train
...
if Terminate_Condition_is_TRUE ==> change to your stopping condition
    ...
    % INSERT YOUR CODE HERE
    % find feature index that makes the best split using q4_rf_split
    % and assign it to feat_selected
	...
	% recursively train left sub-tree
        [tree, next_slot] = q4_rf_train_recursive(X(left_examples, :), Y(left_examples), ...
            feat_idx, tree, slot+1, c, F);
        tree(slot, 3) = next_slot;

	% recursively train right sub-tree
        [tree, next_slot] = q4_rf_train_recursive(X(right_examples, :), Y(right_examples), ...
            feat_idx, tree, next_slot, c, F);
    end
end
